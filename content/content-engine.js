...
class ContentEngine {
  ...
  renderCardGrid(manifest) {
    ...
    const buildGalleryCard = (item, tag, tagColor) => `
      <div class="hub-gallery-card" style="display: flex; flex-direction: column; background: #0d121f; border: 1px solid var(--terminal-border, #1e293b); border-radius: 8px; padding: 24px; transition: all 0.2s ease; cursor: pointer; text-decoration: none;" 
        ...
        <h3 style="font-family: 'Space Mono', monospace; font-size: 1.25rem; font-weight: 600; color: var(--brand-orange, #ff6a00); margin-bottom: 16px; line-height: 1.3;">
          ${item.title}
        </h3>
        
        <p style="color: #94a3b8; font-size: 0.9rem; line-height: 1.6; margin-bottom: 24px; flex-grow: 1;">
  ...
  async loadContent(path, title, pushState = true) {
    ...
    try {
      ...
      ['articles', 'nodes', 'faqs'].forEach(cat => {
      ...
      if (currentItem) {
        metaBadgeHTML = `
          <div style="display: flex; gap: 8px; margin-bottom: 24px; font-family: 'Space Mono', monospace; font-size: 0.65rem;">
            <span style="background: rgba(79, 70, 229, 0.15); color: #818cf8; padding: 4px 10px; border-radius: 4px;">FUNNEL: ${currentItem.funnel || 'General'}</span>
            <span style="background: rgba(255, 106, 0, 0.15); color: var(--brand-orange, #ff6a00); padding: 4px 10px; border-radius: 4px;">DOMAIN: ${currentItem.domain || 'Multi-Industry'}</span>
          </div>
        `;
        ...
        if (currentItem.relations && currentItem.relations.length > 0) {
          ...
          if (relatedItems.length > 0) {
            relationalSuggestionsHTML = `
              <div style="margin-top: 48px; border-top: 1px dashed var(--terminal-border, #1e293b); padding-top: 24px;">
                <h3 style="font-family: 'Space Mono', monospace; font-size: 0.85rem; color: var(--brand-orange, #ff6a00); text-transform: uppercase; margin-bottom: 16px;">
                  &gt; CONNECTED_NODES_IN_ARRAY
                </h3>
                <div style="display: flex; flex-direction: column; gap: 12px;">
                  ${relatedItems.map(r => `
                    <a href="#" onclick="window.contentEngine.loadContent('${r.path}', '${r.title}', true); return false;" style="color: #f8fafc; text-decoration: none; font-size: 0.95rem; font-weight: 500; display: block; padding: 4px 0; transition: color 0.15s;" onmouseover="this.style.color='var(--brand-orange, #ff6a00)'" onmouseout="this.style.color='#f8fafc'">
                      &rarr; ${r.title} <span style="color: var(--terminal-text-muted, #94a3b8); font-size: 0.8rem;">(${r.domain || 'Core'})</span>
                    </a>
                  `).join('')}
                </div>
              </div>
      ...
      if (this.terminalStatus) {
        ...
        this.terminalStatus.style.color = "var(--rate-green, #34d399)";
      }

      // >>> READABILITY OVERHAUL: SCALED PADDING & STRICT SPACING (MIN 16px) <<<
      this.viewport.innerHTML = `
        <style>
          ...
          .rendered-article-container .content-h1 {
            font-family: 'Space Mono', monospace !important;
            color: var(--brand-orange, #ff6a00) !important;
            font-size: 2rem !important;
            margin-bottom: 20px !important;
            line-height: 1.3 !important;
          }
          .rendered-article-container h2, 
          .rendered-article-container .content-h2 {
            font-family: 'Space Mono', monospace !important;
            color: var(--brand-orange, #ff6a00) !important;
          ...
          .rendered-article-container p {
            margin-bottom: 18px !important;
            line-height: 1.7 !important;
            color: #cbd5e1 !important;
          }
          .rendered-article-container ul, 
          .rendered-article-container ol {
            margin-bottom: 20px !important;
            padding-left: 24px !important;
          }
          .rendered-article-container li {
            margin-bottom: 10px !important;
            line-height: 1.6 !important;
            color: #cbd5e1 !important;
          }
          .rendered-article-container table {
            margin-top: 24px !important;
        ...
        <div class="rendered-article-container" style="animation: fadeIn 0.3s ease;">
          <div class="terminal-article-nav" style="margin-bottom: 24px;">
            <button class="btn-terminal-back" onclick="window.contentEngine.restoreProjectDeck(true)" style="cursor:pointer; background: transparent; border: 1px solid var(--terminal-border, #1e293b); color: #f8fafc; padding: 6px 12px; border-radius: 4px; font-family: 'Space Mono', monospace; font-size: 0.75rem;">&larr; Return to Workspace Deck</button>
            <span class="badge-source" style="font-family: 'Space Mono', monospace; font-size: 0.7rem; color: var(--terminal-text-muted, #94a3b8); margin-left: 12px;">SOURCE: ${path.split('/').pop()}</span>
          </div>
          ${metaBadgeHTML}
          ${htmlContent}
          ${relationalSuggestionsHTML}
      ...
      this.viewport.innerHTML = `
        <div style="color: #ef4444; padding: 32px; font-family: 'Space Mono', monospace;">
